...
export class RingBufferUint8Array
{
    ...
    public constructor(sizeBytes: number, flagOverwrite = false)
    {
        if((sizeBytes % 1 !== 0) || !(sizeBytes > 0))
           throw new (class extends Error {
            ...
            })(`RingBufferUint8Array.construct(sizeBytes must be integer > 0, but it "${sizeBytes}")`)
    ...
    public static assertByteToWrite(valueOneByte:number, functionName:string, functionCode:string):void|never
    {
        if(!(valueOneByte <= 255))
            ...
            })(`RingBufferUint8Array.${functionName}(valueOneByte(${valueOneByte}) must be <= 255)`);
        if(!(valueOneByte >= 0))
            throw new (class extends Error {
                name=`${functionCode}_VALUE_LOWER_0`;
            })(`RingBufferUint8Array.${functionName}(valueOneByte(${valueOneByte}) must be >= 0)`);
        if(valueOneByte % 1 !== 0)
            throw new (class extends Error {
                name=`${functionCode}_VALUE_NOT_INTEGER`;
            })(`RingBufferUint8Array.${functionName}(valueOneByte(${valueOneByte}) must be integer)`);
    }
    ...
    public readByteAtBegin(indexAtBegin:number):number|never
    {
        if(!(indexAtBegin <= this._countStoredBytes))
            ...
            })(`RingBufferUint8Array.readByteAtBegin(indexAtBegin(${indexAtBegin}) must be <= ${this._countStoredBytes})`);
    ...
    public readByteAtEnd(indexAtEnd:number):number|never
    {
        if(!(indexAtEnd <= this._countStoredBytes))
            ...
            })(`RingBufferUint8Array.readByteAtEnd(indexAtEnd(${indexAtEnd}) must be <= ${this._countStoredBytes})`);
    ...
    public writeByteAtBegin(indexAtBegin:number, valueOneByte: number):void|never
    {
        if(!(indexAtBegin <= this._countStoredBytes))
            ...
            })(`RingBufferUint8Array.writeByteAtBegin(indexAtBegin(${indexAtBegin}) must be <= ${this._countStoredBytes})`);
        RingBufferUint8Array.assertByteToWrite(valueOneByte, "writeByteAtBegin", "WRITE_BYTE_AT_BEGIN");
    ...
    public writeByteAtEnd(indexAtEnd:number, valueOneByte: number):void|never
    {
        if(!(indexAtEnd <= this._countStoredBytes))
            ...
            })(`RingBufferUint8Array.writeByteAtEnd(indexAtEnd(${indexAtEnd}) must be <= ${this._countStoredBytes})`);
        RingBufferUint8Array.assertByteToWrite(valueOneByte, "writeByteAtEnd", "WRITE_BYTE_AT_END");
    ...
    public shiftByte():number|never
    {
        if(!(this.getCountStoredBytes() > 0))
            ...
            })(`TelemokRingBufferUint8Array.shiftByte no date`);
    ...
    {
        RingBufferUint8Array.assertByteToWrite(valueOneByte, "pushByte", "PUSH_BYTE");
        if(overwrite)
        {
        ...
        else
        {
            if(this._countStoredBytes >= this._bufferSize)
                ...
                })(`RingBufferUint8Array.pushByte() overflow ${this._bufferSize} bytes full`);
    ...
    //     {
    //         if(this._countStoredBytes == this._bufferSize)
    //             throw new ErrorWithCode(`RingBufferUint8Array.pushByte() overflow ${this._bufferSize} bytes full`, `PUSH_BYTE_OVERFLOW`);
    //         this._pushByteNoOverwrite(valueOneByte);
    //     }
    //
    // }
    ...
    //
    //     //valda.uint8Array.assert(uint8Array, "TelemokRingBufferUint8Array.pushUint8Array");
    //     let bytesFree = this.getFreeSpaceBytes();
    //     //if(uint8Array.length > bytesFree)
    //     //	throw new Error(`Can't push uint8Array.length = ${uint8Array.length} bytes to TelemokRingBufferUint8Array with free space = ${bytesFree} bytes!`);
    ...
    // 		throw new Error(`RingBufferUint8Array cant pop ${count} bytes, because stored ${this.getCountStoredBytes()} bytes`);
    // 	this._head -= count;
    // }
